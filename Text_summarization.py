...
def adj(sent_tokens):
    ...
    for sentence in sent_tokens:
        tagged_sent = pos_tag(sentence.split())
        adjective = [word for word, pos in tagged_sent if pos == 'JJ']
        adj[sentence]=len(adjective)
    maximum_frequency = max(adj.values())
    for k in adj.keys():
        try:
            adj[k] = (adj[k]/maximum_frequency)
            adj[k] = round(adj[k], 3)
        except ZeroDivisionError:
            x=0
    print(adj.values())
    print("\n\n Adjectives found :  ", adj)
